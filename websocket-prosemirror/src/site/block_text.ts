...
export class BlockText {
  ...
  private readonly list: List<string | Marker>;
  /** Just the markers from list. */
  private readonly markers: List<Marker>;
  private readonly formatting: TimestampFormatting;
  ...
  constructor(order?: Order) {
    ...
    const initPos: Position = { bunchID: "INIT", innerIndex: 0 };
    const initBlock = { type: "paragraph", attrs: {} };
    this.order.receive([
      { parentID: BunchIDs.ROOT, offset: 1, bunchID: initPos.bunchID },
    ]);
    this.list.set(initPos, initBlock);
    this.markers.set(initPos, initBlock);
  }
